  ...
  return (
    <>
      <section style={{ position: 'relative', minHeight: '42vh', overflow: 'hidden', padding: 'clamp(64px,8vw,96px) var(--pad) clamp(48px,6vw,72px)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div style={{ position: 'absolute', top: '8%', right: 'var(--pad)', zIndex: 2, fontSize: 'clamp(7rem,18vw,16rem)', fontWeight: 200, color: C.paper, opacity: 0.05, letterSpacing: '-0.06em', lineHeight: 1, userSelect: 'none', pointerEvents: 'none' }}>IMAGEN</div>
                  ...
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: 'clamp(28px,3vw,44px) 1fr clamp(60px,8vw,100px)', gap: 'clamp(12px,2vw,28px)', alignItems: 'center', padding: 'clamp(14px,2vw,20px) 0', borderBottom: '0.5px solid '+C.stone }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 12, marginBottom: 8 }}>
                        <span style={{ fontSize: 10, color: C.slate, opacity: 0.65 }}>{fmt(cat.videos,0)} videos</span>
                      </div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div style={{ fontSize: 10, color: C.slate, opacity: 0.5 }}>{pctTotal}% del total</div>
                    </div>
                  </div>
                )
              })}
