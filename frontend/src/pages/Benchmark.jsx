...
export default function Benchmark() {
  ...
  const serieSde = datos.filter(r => r.es_sde)
    .reduce((m,r) => {
      if (!m[r.anio]) m[r.anio] = { anio: r.anio, label: String(r.anio), viajeros: 0 }
    ...
    <>
      <section style={{ position: 'relative', minHeight: '42vh', overflow: 'hidden', padding: 'clamp(64px,8vw,96px) var(--pad) clamp(48px,6vw,72px)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        ...
        <div style={{ position: 'relative', zIndex: 3 }}>
          ...
          <p style={{ fontSize: '0.9rem', fontWeight: 300, color: C.paper, opacity: 0.6, maxWidth: 480, lineHeight: 1.65, margin: 0 }}>SDE frente a otros destinos de la región NOA. Viajeros, pernoctes y estadía media comparados. Fuente: EOH INDEC (hasta nov 2025).</p>
        </div>
      </section>
      ...
      <section style={{ background: C.paper, padding: 'clamp(56px,7vw,80px) var(--pad)' }}>
        ...
        <div className="grid-kpi">
          {[
            ...
            { v: sdePosicion+'° / '+ranking.length, l: 'Posición en ranking NOA', d: 'por viajeros hoteleros' },
            { v: sde_corte.length && sde_corte.reduce((a,b)=>a+b.estadia,0)/sde_corte.length > 0 ? (sde_corte.reduce((a,b)=>a+b.estadia,0)/sde_corte.length).toFixed(1)+' noches' : '—', l: 'Estadía media SDE', d: 'promedio '+anioSel },
            { v: ranking[0]?.localidad || '—', l: 'Líder de la región', d: ranking[0] ? fmt(ranking[0].viajeros)+' viajeros' : '—' },
          ].map((kv,i) => (
        ...
        <Interpretacion texto={'En '+anioSel+', Santiago del Estero (Termas + Capital) acumula '+fmt(sdeViajeros)+' viajeros hoteleros, ocupando el puesto '+sdePosicion+' en la región NOA. Nota: la EOH INDEC fue discontinuada en diciembre 2025 — los datos 2025 corresponden al acumulado hasta noviembre.'} />
      </section>

      {/* RANKING — dark section */}
            ...
            return (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: 'clamp(28px,3.5vw,48px) 1fr clamp(80px,12vw,140px)', gap: 'clamp(12px,2.5vw,36px)', alignItems: 'center', padding: 'clamp(16px,2vw,24px) 0', borderBottom: '0.5px solid rgba(250,250,247,0.08)' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: 'var(--fs-xs)', color: C.stone, opacity: 0.6 }}>{pctTotal}% del total</div>
                </div>
              </div>
            )
          })}
      ...
      {/* TRAYECTORIA — dark section so paper line is visible */}
      <section style={{ background: C.ink, padding: 'clamp(56px,7vw,80px) var(--pad)' }}>
        <SectionTitle icon={ICONS.viajeros} context="Evolución SDE · 2018–2025" main="Trayectoria histórica" light style={{ marginBottom: 40 }} />
        <div style={{ height: 'clamp(180px,22vw,260px)' }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={serieSdeArr} margin={{ top: 8, right: 0, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="viajeros" name="Viajeros SDE" stroke={C.paper} strokeWidth={2.5} dot={{ fill: C.volt, r: 4, strokeWidth: 0 }} activeDot={{ r: 5 }} connectNulls />
            </LineChart>
          </ResponsiveContainer>
        </div>
        <Interpretacion light texto={'La trayectoria de SDE muestra la recuperación post-COVID y el impacto de la discontinuación de la EOH en 2025. Para un benchmark robusto con datos actualizados se requiere el acuerdo con el sector hotelero para recepción directa de estadísticas provinciales.'} />
      </section>
      ...
      <section style={{ background: C.paper, padding: 'clamp(40px,5vw,64px) var(--pad)' }}>
        <Interpretacion>
          ...
          per cápita y la captura de valor. El benchmark interprovincial confirma que
