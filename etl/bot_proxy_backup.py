...
def get_warehouse_data():
    try:
        ...
        ctx['empleo'] = con.execute("SELECT provincia,fecha,empleo_registrado FROM mart_infra_empleo_hyg WHERE provincia LIKE '%Santiago%' ORDER BY fecha DESC LIMIT 12").df().to_dict('records')
        ctx['empleo_noa'] = con.execute("SELECT provincia,fecha,empleo_registrado FROM mart_infra_empleo_hyg WHERE fecha=(SELECT MAX(fecha) FROM mart_infra_empleo_hyg) AND provincia IN ('Santiago del Estero','Tucuman','Salta','Jujuy','Catamarca','La Rioja') ORDER BY empleo_registrado DESC").df().to_dict('records')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        datos_str = json.dumps(Handler.warehouse_data, ensure_ascii=False, default=str)[:25000]
